...
class CallbackContainer(object):
    ...
    def get_topic_sent(self):
        return self._topic_sent

    # Mutator functions
    def reset_setpoint_trigger(self):
        self._setpoint_trigger = False

    def set_topic_sent(self, val):
        self._topic_sent = val
...
while True:
    ...
    if my_callback_container.get_active():
        ...
        # Send shower status topic if water temp is with in +/-0.5*F desired temp
        if (my_callback_container.get_setpoint() - 0.5) <= avg_temp <= (my_callback_container.get_setpoint() + 0.5):
            if not my_callback_container.get_topic_sent():
                my_callback_container.set_topic_sent(True)
                ...
                print('Setpoint reached. Status topic sent.')
